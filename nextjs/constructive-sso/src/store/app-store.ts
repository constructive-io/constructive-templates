...
function createAppStore() {
	...
	/** Cookie-based session auth — no token in localStorage; the httpOnly
	 *  constructive_session cookie authenticates every request via
	 *  credentials:'include'. The token field stays null so getAuthHeaders
	 *  returns {} and the server falls back to the cookie. */
	function setSessionAuthenticated(user: UserProfile) {
		setState({
			auth: { isAuthenticated: true, isLoading: false, user, token: null, rememberMe: false },
		});
	}
...
export const useAuthActions = () => ({
	...
	setSessionAuthenticated: appStore.setSessionAuthenticated,
