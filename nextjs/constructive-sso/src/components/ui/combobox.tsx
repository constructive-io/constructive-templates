  ...
  return (
    <div className="relative not-has-[>*.w-full]:w-fit w-full has-disabled:opacity-64">
      {startAddon && (
        <div
          aria-hidden="true"
          className="[&_svg]:-mx-0.5 pointer-events-none absolute inset-y-0 start-px z-10 flex items-center ps-[calc(--spacing(3)-1px)] has-[+[data-size=sm]]:ps-[calc(--spacing(2.5)-1px)] [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:opacity-80"
      ...
      <ComboboxPrimitive.Input
        className={cn(
          ...
          sizeValue === "sm"
            ? "has-[+[data-slot=combobox-trigger],+[data-slot=combobox-clear]]:*:data-[slot=combobox-input]:pe-6.5 pointer-coarse:has-[+[data-slot=combobox-trigger],+[data-slot=combobox-clear]]:*:data-[slot=combobox-input]:pe-11"
            : "has-[+[data-slot=combobox-trigger],+[data-slot=combobox-clear]]:*:data-[slot=combobox-input]:pe-7 pointer-coarse:has-[+[data-slot=combobox-trigger],+[data-slot=combobox-clear]]:*:data-[slot=combobox-input]:pe-11",
          className,
        )}
        data-slot="combobox-input"
      ...
      {showTrigger && (
        <ComboboxTrigger
          className={cn(
            "-translate-y-1/2 absolute top-1/2 inline-flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-md border border-transparent opacity-80 outline-none transition-opacity pointer-coarse:size-11 sm:pointer-coarse:size-11 hover:opacity-100 has-[+[data-slot=combobox-clear]]:hidden sm:size-7 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
...
const ComboboxContent = ComboboxPopup;

type ComboboxProps<ItemValue = unknown, Multiple extends boolean | undefined = false> =
  ComboboxPrimitive.Root.Props<ItemValue, Multiple>;
type ComboboxTriggerProps = ComboboxPrimitive.Trigger.Props;
type ComboboxContentProps = React.ComponentProps<typeof ComboboxPopup>;
type ComboboxInputProps = React.ComponentProps<typeof ComboboxInput>;
type ComboboxListProps = React.ComponentProps<typeof ComboboxList>;
type ComboboxEmptyProps = React.ComponentProps<typeof ComboboxEmpty>;
type ComboboxGroupProps = React.ComponentProps<typeof ComboboxGroup>;
type ComboboxItemProps = React.ComponentProps<typeof ComboboxItem>;
type ComboboxSeparatorProps = React.ComponentProps<typeof ComboboxSeparator>;
