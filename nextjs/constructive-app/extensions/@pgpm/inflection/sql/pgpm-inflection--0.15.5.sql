...
CREATE FUNCTION inflection.pg_slugify(
  value text,
...
CREATE FUNCTION inflection.pg_slugify(
  text
) RETURNS text AS $EOFCODE$SELECT inflection.pg_slugify($1, false)$EOFCODE$ LANGUAGE sql IMMUTABLE;
...
CREATE FUNCTION inflection.camel(
  str text
) RETURNS text AS $EOFCODE$
DECLARE
  result text[];
...
CREATE FUNCTION inflection.dashed(
  str text
) RETURNS text AS $EOFCODE$
  WITH underscored AS (
    SELECT
...
CREATE FUNCTION inflection.should_skip_uncountable(
  str text
...
CREATE FUNCTION inflection.plural(
  str text
) RETURNS text AS $EOFCODE$
DECLARE
  result record;
...
CREATE FUNCTION inflection.singular(
  str text
) RETURNS text AS $EOFCODE$
DECLARE
  result record;
...
CREATE FUNCTION inflection.slugify(
  value text,
  allow_unicode boolean
) RETURNS text AS $EOFCODE$
  WITH normalized AS (
