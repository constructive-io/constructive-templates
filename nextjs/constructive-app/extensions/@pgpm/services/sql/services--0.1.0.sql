...
CREATE FUNCTION services_private.tg_enforce_api_exposure() RETURNS trigger AS $EOFCODE$
DECLARE
  schema_name text;
...
COMMENT ON FUNCTION services_private.domain_verify(uuid, text[], text, int, uuid) IS 'domain:verify — verify-before-issue guard: advances the outstanding challenge from the DNS/HTTP values a poller observed (observed_values), mirrors verification_status onto managed_domains, and emits verification_started/verified/verification_failed/verification_expired events.';

GRANT EXECUTE ON FUNCTION services_private.domain_verify(uuid, text[], text, int, uuid) TO authenticated;

GRANT EXECUTE ON FUNCTION services_private.domain_verify(uuid, text[], text, int, uuid) TO administrator;
...
CREATE FUNCTION services_private.domain_issue_cert(
  ...
  actor_id uuid DEFAULT NULL
) RETURNS services_public.managed_domains AS $EOFCODE$
DECLARE
...
CREATE FUNCTION services_private.tg_domain_enqueue_verify_confirm() RETURNS trigger AS $EOFCODE$
BEGIN
...
CREATE TRIGGER _000010_domain_enqueue_verify_confirm
  AFTER INSERT
  ON services_public.domain_verifications
  FOR EACH ROW
  WHEN (new.status = 'pending')
  EXECUTE PROCEDURE services_private.tg_domain_enqueue_verify_confirm();

CREATE FUNCTION services_private.tg_domain_enqueue_issue_cert() RETURNS trigger AS $EOFCODE$
BEGIN
  PERFORM app_jobs.add_job(
...
CREATE TRIGGER _000011_domain_enqueue_issue_cert
  AFTER UPDATE
  ON services_public.managed_domains
  FOR EACH ROW
  WHEN (new.verification_status = 'verified'
    AND new.verification_status IS DISTINCT FROM old.verification_status)
  EXECUTE PROCEDURE services_private.tg_domain_enqueue_issue_cert();

CREATE FUNCTION services_private.tg_domain_enqueue_cert_confirm() RETURNS trigger AS $EOFCODE$
BEGIN
  PERFORM app_jobs.add_job(
    identifier := 'infra:confirm',
    payload := json_build_object(
...
CREATE TRIGGER _000012_domain_enqueue_cert_confirm
  ...
  EXECUTE PROCEDURE services_private.tg_domain_enqueue_cert_confirm();