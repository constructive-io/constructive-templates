...
CREATE TRIGGER catalog_del
  ...
  ON routing_public.platform_apis
  FOR EACH ROW
  EXECUTE PROCEDURE catalog_public.tg_platform_apis_catalog_del();
...
COMMENT ON COLUMN routing_public.platform_sites.config IS 'Module-specific configuration for this site surface';
...
CREATE INDEX platform_site_modules_site_id_idx ON routing_public.platform_site_modules (site_id);
...
CREATE TRIGGER catalog_sync
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE catalog_public.tg_platform_sites_catalog_sync();

CREATE TRIGGER catalog_del
  AFTER DELETE
  ON routing_public.platform_sites
  FOR EACH ROW
  EXECUTE PROCEDURE catalog_public.tg_platform_sites_catalog_del();
...
ALTER TABLE platform_routing_public.platform_routes 
  ADD CONSTRAINT platform_routes_target_api_id_fkey
    ...
    REFERENCES catalog_public.apis (id)
    ON DELETE CASCADE;
