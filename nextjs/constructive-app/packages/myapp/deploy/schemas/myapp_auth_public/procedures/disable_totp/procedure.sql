-- Deploy: schemas/myapp_auth_public/procedures/disable_totp/procedure
...
CREATE FUNCTION myapp_auth_public.disable_totp(
  IN totp_value text
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('disable_totp'), hashtext(v_user_id::text));
  ...
  WHERE
    subject_id = v_user_id AND action = 'disable_totp' INTO v_user_rate_limit;
  ...
  END IF;
  IF totp.verify(v_totp_secret, disable_totp.totp_value, 30, 6) IS TRUE THEN
    DELETE FROM myapp_auth_private.auth_rate_limits
    WHERE
      subject_id = v_user_id AND action = 'disable_totp';
    PERFORM myapp_store_private.user_state_del(v_user_id, ARRAY['totp_secret', 'totp_setup_secret']);
  ...
  ELSE
    ...
    VALUES
      (v_user_id, 'disable_totp', 1, now(), now(), NULL)
