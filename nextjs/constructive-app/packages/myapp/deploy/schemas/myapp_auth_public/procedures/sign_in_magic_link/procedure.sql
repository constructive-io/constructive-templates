-- Deploy: schemas/myapp_auth_public/procedures/sign_in_magic_link/procedure
...
CREATE FUNCTION myapp_auth_public.sign_in_magic_link(
  IN token text,
...
BEGIN
  ...
  IF v_ip_address IS NOT NULL THEN
    ...
    PERFORM pg_advisory_xact_lock(hashtext('ip_rate_limit_sign_in_magic_link'), hashtext(v_ip_address::text));
    IF EXISTS (SELECT 1
    FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      ((ip_address = v_ip_address AND ua_hash = ANY( ARRAY[v_ua_hash, ''] )) AND action = 'sign_in_magic_link') AND locked_until > now()
    LIMIT
    1) THEN
  ...
  END IF;
  IF v_settings.allowed_auth_methods IS NOT NULL AND NOT ('magic_link' = ANY( v_settings.allowed_auth_methods )) THEN
  ...
  v_token_hash := encode(digest(sign_in_magic_link.token, 'sha256'), 'hex');
  v_token_email := myapp_store_private.user_state_get(uuid_nil(), concat('magic_link_email:', v_token_hash));
  IF v_token_email IS NULL THEN
    PERFORM errors.raise_error('INVALID_TOKEN', '{}', 'public');
  END IF;
  SELECT *
  FROM myapp_user_identifiers_public.emails AS t
  WHERE
    (trim(v_token_email))::email = t.email INTO v_email;
  IF v_email.owner_id IS NULL THEN
  ...
  END IF;
  IF COALESCE(v_settings.enforce_primary_auth_method, true) AND myapp_store_private.user_state_get(v_user_id, 'primary_auth_method') <> 'magic_link' THEN
  ...
  IF v_device_settings.enable_device_tracking IS TRUE THEN
    IF sign_in_magic_link.device_token IS NOT NULL THEN
      SELECT digest(sign_in_magic_link.device_token, 'sha256') INTO v_device_token_hash;
      SELECT *
  ...
  IF v_device_settings.require_device_approval IS TRUE AND v_device_approved IS NOT TRUE THEN
    ...
    SELECT
      COALESCE(v_new_device_token, sign_in_magic_link.device_token) INTO out_device_token;
    RETURN;
  END IF;
  PERFORM myapp_store_private.user_state_del(uuid_nil(), concat('magic_link_email:', v_token_hash));
  ...
  VALUES
    (v_user_id, 'sign_in_magic_link', true);
  v_csrf_secret := encode(gen_random_bytes(32), 'hex');
  v_session_id := uuidv7();
  IF sign_in_magic_link.remember_me IS TRUE THEN
  ...
  VALUES
    (v_session_id, v_user_id, false, v_session_expires_at, 'magic_link', v_csrf_secret, jwt_public.current_origin(), jwt_public.current_user_agent());
  v_plaintext_credential := (CASE 
    WHEN sign_in_magic_link.credential_kind = 'api_key' THEN 'cnc_live_sk_' 
    WHEN sign_in_magic_link.credential_kind = 'bearer' THEN 'cnc_live_bt_' 
    WHEN sign_in_magic_link.credential_kind = 'access_token' THEN 'cnc_live_at_' 
    WHEN sign_in_magic_link.credential_kind = 'mfa_challenge' THEN 'cnc_live_mfa_' 
    WHEN sign_in_magic_link.credential_kind = 'one_time' THEN 'cnc_live_ot_' 
    WHEN sign_in_magic_link.credential_kind = 'webauthn' THEN 'cnc_live_wa_' 
    ELSE 'cnc_live_tk_' 
  ...
  VALUES
    (v_credential_id, v_session_id, sign_in_magic_link.credential_kind, digest(v_plaintext_credential, 'sha256'), v_session_expires_at);
  SELECT v_user_id INTO user_id;
  ...
  IF v_ip_address IS NOT NULL THEN
    ...
    WHERE
      (ip_address = v_ip_address AND ua_hash = v_ua_hash) AND action = 'sign_in_magic_link';
    DELETE FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      (ip_address = v_ip_address AND ua_hash = '') AND action = 'sign_in_magic_link';
  END IF;
  RETURN;
END;
