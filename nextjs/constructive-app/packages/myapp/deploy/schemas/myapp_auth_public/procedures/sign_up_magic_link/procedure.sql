-- Deploy: schemas/myapp_auth_public/procedures/sign_up_magic_link/procedure
...
CREATE FUNCTION myapp_auth_public.sign_up_magic_link(
  IN token text,
...
BEGIN
  ...
  IF v_ip_address IS NOT NULL THEN
    ...
    PERFORM pg_advisory_xact_lock(hashtext('ip_rate_limit_sign_up_magic_link'), hashtext(v_ip_address::text));
    IF EXISTS (SELECT 1
    FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      ((ip_address = v_ip_address AND ua_hash = ANY( ARRAY[v_ua_hash, ''] )) AND action = 'sign_up_magic_link') AND locked_until > now()
    LIMIT
    1) THEN
  ...
  END IF;
  IF NOT (COALESCE(v_settings.allow_magic_link_sign_up, false)) THEN
    PERFORM errors.raise_error('MAGIC_LINK_SIGN_UP_DISABLED', '{}', 'public');
  END IF;
  ...
  v_token_hash := encode(digest(sign_up_magic_link.token, 'sha256'), 'hex');
  ...
  END IF;
  v_user_id := myapp_auth_public.provision_new_user(v_token_email);
  PERFORM myapp_store_private.user_state_set(v_user_id, 'primary_auth_method', 'magic_link'::text);
  ...
  v_session_id := uuidv7();
  IF sign_up_magic_link.remember_me IS TRUE THEN
  ...
  v_plaintext_credential := (CASE 
    WHEN sign_up_magic_link.credential_kind = 'api_key' THEN 'cnc_live_sk_' 
    WHEN sign_up_magic_link.credential_kind = 'bearer' THEN 'cnc_live_bt_' 
    WHEN sign_up_magic_link.credential_kind = 'access_token' THEN 'cnc_live_at_' 
    WHEN sign_up_magic_link.credential_kind = 'mfa_challenge' THEN 'cnc_live_mfa_' 
    WHEN sign_up_magic_link.credential_kind = 'one_time' THEN 'cnc_live_ot_' 
    WHEN sign_up_magic_link.credential_kind = 'webauthn' THEN 'cnc_live_wa_' 
    ELSE 'cnc_live_tk_' 
  ...
  VALUES
    (v_credential_id, v_session_id, sign_up_magic_link.credential_kind, digest(v_plaintext_credential, 'sha256'), v_session_expires_at);
  ...
  IF v_device_settings.enable_device_tracking IS TRUE THEN
    IF sign_up_magic_link.device_token IS NOT NULL THEN
      SELECT digest(sign_up_magic_link.device_token, 'sha256') INTO v_device_token_hash;
    ELSE
  ...
  END IF;
  SELECT
    COALESCE(v_new_device_token, sign_up_magic_link.device_token) INTO out_device_token;
  IF v_ip_address IS NOT NULL THEN
    DELETE FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      (ip_address = v_ip_address AND ua_hash = v_ua_hash) AND action = 'sign_up_magic_link';
    DELETE FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      (ip_address = v_ip_address AND ua_hash = '') AND action = 'sign_up_magic_link';
  END IF;
  RETURN;
END;
