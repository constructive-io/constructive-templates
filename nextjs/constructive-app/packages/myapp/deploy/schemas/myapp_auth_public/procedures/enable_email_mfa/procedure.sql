-- Deploy: schemas/myapp_auth_public/procedures/enable_email_mfa/procedure
...
CREATE FUNCTION myapp_auth_public.enable_email_mfa() RETURNS boolean AS $_PGFN_$
DECLARE
  v_user_id uuid;
BEGIN
  ...
  email_mfa_enabled = true, mfa_enrolled_at = COALESCE(mfa_enrolled_at, CURRENT_TIMESTAMP)
  WHERE
    owner_id = v_user_id;
  ...
  VALUES
    (v_user_id, 'enable_email_mfa', true);
  RETURN true;
END;
