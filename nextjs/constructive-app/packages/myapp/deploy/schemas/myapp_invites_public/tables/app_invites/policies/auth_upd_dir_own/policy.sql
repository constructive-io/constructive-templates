-- Deploy: schemas/myapp_invites_public/tables/app_invites/policies/auth_upd_dir_own/policy
-- made with <3 @ constructive.io

-- requires: schemas/myapp_invites_public/schema
-- requires: schemas/myapp_invites_public/tables/app_invites/table
-- requires: schemas/myapp_invites_public/tables/app_invites/policies/enable_row_level_security


CREATE POLICY auth_upd_dir_own ON myapp_invites_public.app_invites
FOR UPDATE
TO authenticated
USING (
  sender_id = jwt_public.current_principal_id()
);

