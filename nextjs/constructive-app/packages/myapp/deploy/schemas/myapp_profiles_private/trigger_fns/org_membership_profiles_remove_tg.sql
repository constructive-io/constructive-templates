-- Deploy: schemas/myapp_profiles_private/trigger_fns/org_membership_profiles_remove_tg
...
CREATE FUNCTION myapp_profiles_private.org_membership_profiles_remove_tg() RETURNS TRIGGER AS $_PGFN_$
BEGIN
  ...
  WHERE
    mp.membership_id = OLD.membership_id AND mp.profile_id = org_memberships.profile_id), (SELECT mp.profile_id
  FROM myapp_profiles_public.org_membership_profiles AS mp
  WHERE
