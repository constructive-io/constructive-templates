-- Deploy: schemas/myapp_profiles_private/trigger_fns/org_profile_capabilities_recompute_tg
...
CREATE FUNCTION myapp_profiles_private.org_profile_capabilities_recompute_tg() RETURNS TRIGGER AS $_PGFN_$
DECLARE
  v_profile_id uuid;
...
BEGIN
  ...
  FROM myapp_profiles_public.org_profile_capabilities AS pp INNER JOIN myapp_capabilities_public.org_capabilities AS p ON p.id = pp.capability_id
  WHERE
