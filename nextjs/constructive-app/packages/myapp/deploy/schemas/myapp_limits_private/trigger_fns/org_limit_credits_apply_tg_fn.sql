...
BEGIN
  ...
  IF NEW.credit_type = 'permanent' THEN
    ...
    purchased_credits = purchased_credits + NEW.amount, max = ((CASE 
      WHEN (COALESCE(org_limits.plan_max, (SELECT d.max
    FROM myapp_limits_public.org_limit_defaults AS d
    WHERE
        d.name = org_limits.name), 0)) < 0 THEN -1 
      ELSE (COALESCE(org_limits.plan_max, (SELECT d.max
    FROM myapp_limits_public.org_limit_defaults AS d
    WHERE
        d.name = org_limits.name), 0)) + org_limits.purchased_credits 
