...
BEGIN
  ...
  WHERE
    ((l.name = org_limits_inc.limitname AND l.actor_id = org_limits_inc.actor_id) AND l.entity_id = org_limits_inc.entity_id) AND ((CASE 
      ...
      END) < 0 OR (CASE 
        WHEN (COALESCE(l.plan_max, max_default, 0)) < 0 THEN -1 
        ELSE ((COALESCE(l.plan_max, max_default, 0)) + l.purchased_credits) + (COALESCE(l.period_credits, 0)) 
      END) >= (l.num + org_limits_inc.amount));
  IF FOUND THEN
    RETURN true;
  ELSE
    RETURN false;
