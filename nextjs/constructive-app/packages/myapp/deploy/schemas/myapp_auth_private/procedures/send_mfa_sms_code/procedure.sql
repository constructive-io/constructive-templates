-- Deploy: schemas/myapp_auth_private/procedures/send_mfa_sms_code/procedure
...
CREATE FUNCTION myapp_auth_private.send_mfa_sms_code(
  IN user_id uuid
...
BEGIN
  ...
  IF v_ip_address IS NOT NULL THEN
    ...
    PERFORM pg_advisory_xact_lock(hashtext('ip_rate_limit_mfa_sms_code'), hashtext(v_ip_address::text));
    IF EXISTS (SELECT 1
    FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      ((ip_address = v_ip_address AND ua_hash = ANY( ARRAY[v_ua_hash, ''] )) AND action = 'mfa_sms_code') AND locked_until > now()
    LIMIT
    1) THEN
      PERFORM errors.raise_error('TOO_MANY_REQUESTS', '{}', 'public');
    END IF;
  END IF;
  v_user_id := send_mfa_sms_code.user_id;
  IF v_user_id IS NULL THEN
    PERFORM errors.raise_error('INVALID_USER', '{}', 'public');
  END IF;
  PERFORM pg_advisory_xact_lock(hashtext('send_mfa_sms_code'), hashtext(v_user_id::text));
  SELECT *
  FROM myapp_auth_private.auth_rate_limits
  WHERE
    subject_id = v_user_id AND action = 'send_mfa_sms_code' INTO v_user_rate_limit;
  ...
  WHERE
    pn.owner_id = v_user_id AND pn.is_primary = true INTO v_phone;
  IF NOT (FOUND) THEN
    RETURN false;
  END IF;
  v_sms_mfa_secret := myapp_store_private.user_state_get(v_user_id, 'sms_mfa_secret');
  IF v_sms_mfa_secret IS NULL THEN
    v_sms_mfa_secret := encode(gen_random_bytes(20), 'hex');
    PERFORM myapp_store_private.user_state_set(v_user_id, 'sms_mfa_secret', v_sms_mfa_secret);
  END IF;
  v_code := totp.generate(v_sms_mfa_secret, 600, 6);
  PERFORM app_jobs.add_job('sms:send_verification_code', json_build_object('sms_type', 'mfa_verification_code', 'user_id', v_user_id::text, 'phone_cc', v_phone.cc, 'phone_number', v_phone.number, 'code', v_code));
  ...
  VALUES
    (v_user_id, 'send_mfa_sms_code', true);
  IF v_ip_address IS NOT NULL THEN
    DELETE FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      (ip_address = v_ip_address AND ua_hash = v_ua_hash) AND action = 'mfa_sms_code';
    DELETE FROM myapp_auth_private.auth_ip_rate_limits
    WHERE
      (ip_address = v_ip_address AND ua_hash = '') AND action = 'mfa_sms_code';
  END IF;
  RETURN true;
END;
