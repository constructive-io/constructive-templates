...
import {
	fetchAppClaimedInvitesQuery,
	fetchAppInvitesQuery,
...
export function useAppInvites(options: UseAppInvitesOptions = {}) {
	const { enabled = true, first = 20, offset = 0, context = 'admin' } = options;
	...
	const { data, isLoading, error, refetch } = useQuery<AppInvitesQueryResult>({
		...
		queryFn: async (): Promise<AppInvitesQueryResult> => {
			...
			const invitesResult = await fetchAppInvitesQuery({
				selection: {
					fields: {
						id: true,
						email: true,
			...
			const rawInvites = invitesResult.appInvites?.nodes ?? [];
			const totalCount = invitesResult.appInvites?.totalCount ?? 0;
			const pageInfo = invitesResult.appInvites?.pageInfo ?? {
				hasNextPage: false,
...
export function useAppClaimedInvites(options: UseAppInvitesOptions = {}) {
	...
	const { data, isLoading, error, refetch } = useQuery<AppClaimedInvitesQueryResult>({
		...
		queryFn: async (): Promise<AppClaimedInvitesQueryResult> => {
			...
			const claimedResult = await fetchAppClaimedInvitesQuery({
				selection: {
					fields: {
						id: true,
						data: true,
			...
			const rawClaimed = claimedResult.appClaimedInvites?.nodes ?? [];
			const totalCount = claimedResult.appClaimedInvites?.totalCount ?? 0;
			const pageInfo = claimedResult.appClaimedInvites?.pageInfo ?? {
				hasNextPage: false,
				hasPreviousPage: false,
			};
...
export function useSendAppInvite(defaultContext: SchemaContext = 'admin') {
	void defaultContext; // Context handled by SDK execute-adapter
	const queryClient = useQueryClient();
	const createMutation = useCreateAppInviteMutation({ selection: { fields: { id: true } } });
	...
	return {
		sendInvite: async (input: SendAppInviteInput) => {
			...
			return result.createAppInvite?.appInvite ?? null;
		},
...
export function useCancelAppInvite(defaultContext: SchemaContext = 'admin') {
	void defaultContext; // Context handled by SDK execute-adapter
	const queryClient = useQueryClient();
	const deleteMutation = useDeleteAppInviteMutation({ selection: { fields: { id: true } } });
...
export function useExtendAppInvite(defaultContext: SchemaContext = 'admin') {
	void defaultContext; // Context handled by SDK execute-adapter
	const queryClient = useQueryClient();
	const updateMutation = useUpdateAppInviteMutation({ selection: { fields: { id: true } } });
	...
	return {
		extendInvite: async (input: ExtendAppInviteInput) => {
			...
			queryClient.invalidateQueries({ queryKey: appInvitesQueryKeys.byContext(ctx) });
			return result.updateAppInvite?.appInvite ?? null;
		},
